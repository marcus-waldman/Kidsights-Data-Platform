/**
 * Authenticity Screening GLMM - Two-Dimensional IRT with Skewness Penalty (INDEPENDENT VERSION)
 *
 * Identical to authenticity_glmm_skewpenalty.stan except:
 *   - Person effects are INDEPENDENT between dimensions (no correlation)
 *   - eta[i, 1] ~ N(0, 1) and eta[i, 2] ~ N(0, 1) independently
 *   - Removes Cholesky decomposition and correlation estimation
 *
 * Use this version if:
 *   - Correlation estimation is unstable or causing convergence issues
 *   - You want a simpler model as a diagnostic baseline
 *   - Computational efficiency is critical (~5,200 vs ~8,200 parameters)
 *
