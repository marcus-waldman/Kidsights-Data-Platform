/**
 * Authenticity Screening GLMM - Two-Dimensional IRT Model with LKJ Prior (TEST VERSION)
 *
 * This is a test model that allows correlation between psychosocial and developmental
 * dimensions using an LKJ prior. Compare to authenticity_glmm.stan (independent model).
 *
 * Model:
 ...
 *   L_Omega ~ lkj_corr_cholesky(10)  [concentrates around independence]
 *   eta_std ~ N(0, 1) [standardized, then transformed to correlated with var=1]
 *
 * Design Notes:
 *   - LKJ(10) prior concentrates probability near correlation = 0 (favors independence)
 *   - LKJ(1) would be uniform over all correlations
...
parameters {
  ...
  real<lower=0> delta;         // Threshold spacing (positive, shared across dimensions)
...
model {
  ...
  L_Omega ~ lkj_corr_cholesky(10);  // Concentrates around independence (eta=10)
