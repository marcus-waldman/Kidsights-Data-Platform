/**
 * Authenticity Screening GLMM - Two-Dimensional IRT with Skewness Penalty (SOFT-CLIPPED + SUM PRIOR)
 *
 ...
 *   - **NEW**: Prior on sum(softwgt) ~ N(N, sigma_sum_softwgt) to prevent degeneracy
 ...
 *
 * Sum Prior Rationale:
 *   - Dirichlet(lambda_wgt < 1) encourages sparsity (concentration on few participants)
 *   - Soft-clipping reduces sum(softwgt) when weights get large
 *   - Prior sum(softwgt) ~ N(N, sigma) creates tension toward uniform weighting
 *   - Prevents degeneracy: model can't concentrate all weight on 1-2 participants
 *   - Allows moderate down-weighting: sigma_sum_softwgt controls tolerance
 *
 ...
 *   - Balances sparsity (Dirichlet) vs uniformity (sum prior)
 *
 * Design Philosophy:
 ...
 *   - Sum prior prevents over-concentration on tiny subsets
 *
 * Key Features:
 ...
 *   - Sum prior: normal_lpdf(sum(softwgt) | N, sigma_sum_softwgt)
 *
 * Hyperparameters:
 ...
 *     - Small (1-5): Tight prior, strong preference for sum ≈ N (near-uniform)
 *     - Medium (5-10): Moderate tolerance, allows ~10-20% reduction in sum
 *     - Large (>10): Weak prior, allows substantial down-weighting
 */
...
model {
...

  {
    ...
    // This creates tension with Dirichlet sparsity, preventing extreme concentration
    // Allows moderate down-weighting but prevents model from excluding most participants
