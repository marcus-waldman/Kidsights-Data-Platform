/**
 * Authenticity Screening GLMM - Two-Dimensional IRT with Skewness Penalty (BETA + SUM PRIOR)
 *
 * Key differences from standard Beta version:
 *   - Uses INDEPENDENT Beta(alpha_wgt, beta_wgt) priors on each weight
 *   - No simplex constraint - weights can vary freely in [0, 1]
 *   - **NEW**: Prior on sum(w) ~ N(N, sigma_sum_w) to prevent degeneracy
 *   - Skewness penalty uses sum(w) explicitly (not assuming sum = N)
 *   - INDEPENDENT person effects (no correlation between dimensions)
 *
 * Sum Prior Rationale:
 *   - Beta(alpha, beta) prior allows each weight to vary independently
 *   - Without constraint, sum(w) can be very small or very large
 *   - Prior sum(w) ~ N(N, sigma) regularizes toward mean weight ≈ 1
 ...
 *   - Balances individual weight flexibility (Beta) vs collective constraint (sum prior)
 *
 * Design Philosophy:
 ...
 *   - Beta prior controls individual weight distribution
 *   - Sum prior prevents pathological collective behavior
 *
 * Key Features:
 ...
 *   - Sum regularization: normal_lpdf(sum(w) | N, sigma_sum_w)
 *
 * Hyperparameters:
 ...
 *   - sigma_sum_w: Prior SD for sum(w) ~ N(N, sigma)
...
data {
  ...
  real<lower=0> sigma_sum_w;   // Prior SD for sum(w) ~ N(N, sigma)
}
...
parameters {
  ...
  vector<lower=0, upper=1>[N] w;  // Weights in [0, 1], sum NOT constrained
}
...
model {
...

  {
    ...
    // ============================================================================
    // SUM PRIOR: Prevent degeneracy by constraining sum(w) near N
    // ============================================================================

    // Prior: sum(w) ~ N(N, sigma_sum_w)
    // This creates tension with Beta prior flexibility
    // Prevents pathological solutions where sum(w) << N or sum(w) >> N
...
generated quantities {
  ...
  real sum_weight = sum_w_final;  // Regularized toward N
