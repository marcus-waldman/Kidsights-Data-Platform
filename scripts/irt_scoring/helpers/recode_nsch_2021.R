...
#'   - id: Numeric composite ID (format: 77210XXXXXX for NSCH 2021)
#'   - years: Child age in years (calculated from birth date)
#'   - study: Character "NSCH21"
...
#' 6. Creates composite ID with study indicator (7721)
#'
...
recode_nsch_2021 <- function(codebook_path = "codebook/data/codebook.json",
  ...
  for (item_id in names(codebook$items)) {
    ...
    if (!is.null(item$lexicons$cahmi21) && nchar(item$lexicons$cahmi21) > 0) {
      cahmi21_mappings[[item$lexicons$cahmi21]] <- item$lexicons$equate
    }
  }
  ...
  select_cols <- c("YEAR", "BIRTH_YR", "BIRTH_MO", "SC_AGE_YEARS", available_cahmi21_vars)
  ...
  nsch21_filtered <- nsch21 %>%
    ...
    dplyr::select(years, SC_AGE_YEARS, dplyr::any_of(available_cahmi21_vars)) %>%
  ...
  # Forward-coded items (higher values = more severe/advanced)
  forwardly_coded21 <- c("DISTRACTED", "COUNTTO", "TEMPER", "NEWACTIVITY")
  forwardly_coded21 <- intersect(forwardly_coded21, available_cahmi21_vars)

  # All other items are reverse-coded
  reverse_coded21 <- setdiff(available_cahmi21_vars, forwardly_coded21)
  ...
  item_counts <- nsch21_filtered %>%
    dplyr::select(dplyr::any_of(available_cahmi21_vars)) %>%
  ...
  # ============================================================================
  # Map CAHMI21 Names to lex_equate Names
  # ============================================================================
  ...
  # Create composite ID (format: 77 + 21 + 6-digit row number)
  nsch21_final <- nsch21_final %>%
    dplyr::mutate(
      id = as.numeric(paste0("7721", stringr::str_pad(rid, 6, side = "left", pad = "0"))),
      study = "NSCH21"
    ) %>%
  ...
  item_cols <- setdiff(names(nsch21_final), c("id", "study", "years"))
