...
cat(sprintf("  Total imputed variables: %d\n",
            length(config$geography$variables) + length(config$sociodemographic$variables) + 4))
...
cat(sprintf("     df = get_completed_dataset(m=1, variables=['female', 'raceG', 'puma', 'cc_receives_care'], study_id='%s')\n", study_id))
cat("  3. Run analysis with multiply imputed data (including childcare variables)\n")
