# Calibration Estimator Wrapper: Factorized Covariance (Masked KL Divergence)
# Purpose: Estimate survey weights with incomplete covariance structure
# Uses: Stan optimization with binary mask for observed covariance elements
#
# Key Innovation: Handles multi-source data where some covariances unobserved
#   - Example: PUMA (ACS) × Mental Health (NHIS) = unobserved
#   - Solution: cov_mask matrix specifies observed (1) vs unobserved (0) elements
...
# ============================================================================
# Main Calibration Function (Factorized Covariance)
# ============================================================================

calibrate_weights_factorized_stan <- function(data, target_mean, target_cov, cov_mask,
                                              calibration_vars, verbose = TRUE) {

  cat("\n========================================\n")
  cat("Calibrating Survey Weights (Factorized Covariance)\n")
  cat("(Masked KL divergence minimization via Stan optimization)\n")
  cat("========================================\n\n")
  ...
  stan_data <- list(
    ...
    cov_mask = cov_mask  # NEW: Binary mask for observed covariances
  )

  # Compile Stan model
  cat("[7] Compiling Stan model (factorized covariance version):\n\n")

  stan_file <- "scripts/raking/ne25/utils/calibrate_weights_factorized.stan"
  ...
  cat("[8] Running Stan optimizer (BFGS with masked covariance):\n\n")
  ...
  cat("[10] Extracting optimal parameters:\n\n")
  ...
  # Return results
  cat("========================================\n")
  cat("Calibration Complete (Factorized Covariance)\n")
  cat("========================================\n\n")
