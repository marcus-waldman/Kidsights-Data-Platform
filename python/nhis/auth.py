"""
IPUMS Health Surveys API Authentication Module

Handles IPUMS API key management and client initialization for the NHIS pipeline.
Uses the same IPUMS API infrastructure as ACS, but targets the NHIS collection.
...
def initialize_ipums_client(
    ...
    Note:
        The same IPUMS API client works for both NHIS and ACS data.
        Collection selection (nhis vs usa) is handled during extract building.

    Example:
        >>> client = initialize_ipums_client()
        >>> # Client is ready to use for NHIS extract requests
    """
    ...
    # Initialize client
    log.debug("Initializing IPUMS API client for NHIS")

    try:
        client = IpumsApiClient(api_key)
        log.info("IPUMS API client initialized successfully for NHIS")
        return client
        ...
        raise Exception(
            ...
            f"  2. Check you have registered for IPUMS NHIS: https://nhis.ipums.org/\n"
...
def get_ipums_client(
    ...
    Example:
        ...
        >>> client1 = get_ipums_client()
        >>>
        >>> # Subsequent calls return cached client (fast)
        >>> client2 = get_ipums_client()
...
def clear_client_cache():
    ...
    Example:
        >>> client1 = get_ipums_client()
        >>> clear_client_cache()
        >>> client2 = get_ipums_client()  # Will create new client
