...
This document provides detailed architecture documentation for the **eight data pipelines** in the Kidsights Data Platform. Each pipeline is designed as an independent, standalone system with specific data sources and use cases.

> **Note:** This doc still covers the original 7 pipelines in detail. The **MN26 pipeline** was added April 2026 — see [docs/mn26/pipeline_guide.qmd](../mn26/pipeline_guide.qmd) for its full architecture (not duplicated here).
...
2. [MN26 Pipeline](../mn26/pipeline_guide.qmd) - Minnesota 2026 multi-child REDCap survey *(see linked Quarto guide for architecture)*
3. [ACS Pipeline](#acs-pipeline) - Census data extraction for statistical raking
4. [NHIS Pipeline](#nhis-pipeline) - National health surveys for benchmarking
5. [NSCH Pipeline](#nsch-pipeline) - Children's health survey integration
6. [Raking Targets Pipeline](#raking-targets-pipeline) - Population-representative targets for post-stratification
7. [Imputation Pipeline](#imputation-pipeline) - Multiple imputation for geographic, sociodem, childcare, mental health, child ACEs uncertainty
8. [IRT Calibration Pipeline](#irt-calibration-pipeline) - Mplus calibration dataset for psychometric recalibration
9. [ACS Metadata System](#acs-metadata-system) - IPUMS DDI metadata integration
10. [Pipeline Integration](#pipeline-integration-and-relationship) - How pipelines work together
...
- **Storage:** Local DuckDB database (~60 `ne25_*` tables, 97 total tables in DB; verified 2026-04-20)
...
### Eight Independent Pipelines

The Kidsights Data Platform operates **eight independent, standalone pipelines** (see [CLAUDE.md](../../CLAUDE.md) for current per-pipeline status):
...
- **Status:** Production ready (180 targets generated by design; *⚠️ as of 2026-04-20 the `raking_targets_ne25` table contains only 11 rows — investigate if this is expected after a rebuild*)
...
- **Status:** Production ready (M=5 imputations, 29 variables, ~3 min). *Note: 34 `ne25_imputed_*` tables in current DB (incl. 4 likely-defunct double-prefixed `ne25_imputed_imputed_cc_*` with 0 rows) — verified 2026-04-20.*
- **Tables:** ~30 tables per study: `{study_id}_imputed_{variable}` (3 geography + 6 sociodem + 4 childcare + 7 mental health/parenting + 9 child ACEs)
...
calibration/ne25/manual_2023_scale/run_manual_calibration.R
         ↓
ne25_kidsights_gsed_pf_scores_2022_scale (2,785 records — verified 2026-04-20)
...
ne25_transformed (+ GSED scores joined in Step 6.7)
     ...
     ↓                                      ↓
     ↓                            raking_targets_ne25
     ↓                  (180 population targets by design;
     ↓                   ⚠️ DB has 11 rows as of 2026-04-20)
     ↓                                      ↓
     └──────────────────────────────────────┘
                           ↓
...
**Status:** Implemented and operational *(⚠️ DB drift as of 2026-04-20: see note above)*
...
# Run full 11-stage pipeline (geography + sociodem + childcare + mental health + child ACEs)
...
### Corrections applied
- Pipeline count: "seven" → "eight" (intro), added MN26 to TOC pointing to per-pipeline guide
- "Six Independent Pipelines" → "Eight Independent Pipelines"
- Storage claim "47MB, 11 tables, 7,812 records" → "~60 ne25_* tables, 97 total"
- GSED person-fit scores: 2,831 → 2,785
- Imputation: "30 variables" → "29 variables" (with note about 34 tables incl. 4 defunct)
- "7-stage pipeline" → "11-stage pipeline"
- Raking targets section flagged with 180-vs-11 DB drift caveat
- Last Updated date refreshed

### Known remaining drift (not surgically fixed in this pass)
- The "Pipeline Stages" overview lists "13 steps" but the NE25 pipeline now has additional steps (Step 7.5 CREDI, Step 7.6 D-score, Step 7.7 HRTL) per CLAUDE.md
- MN26 pipeline architecture is referenced in the TOC but not duplicated here — see [`docs/mn26/pipeline_guide.qmd`](../mn26/pipeline_guide.qmd)
- Sociodem variable list in the integration diagram still shows 7 (incl. fplcat); CLAUDE.md says 6 sociodem
- Various stage-row counts (25,480 / 26,438 / 24,718) are point-in-time snapshots from October 2025
